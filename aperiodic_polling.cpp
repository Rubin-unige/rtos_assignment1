...
//code of periodic tasks, total 4
void task1_code( );
void task2_code( );
void task3_code( );
void task4_code( ); // added periodic task, used task 4 for better readability and understandability

//code of aperiodic tasks, total 3 
...
#define NAPERIODICTASKS 3 // with new task
...
main()
{
	...
	periods[4]= 800000000; //in nanoseconds, keeping harmonic relations , ulub < 1
...
//funcitons allocated to periodic task 4
void task4_code()
{
	//print the id of the current task
...
void *task4( void *ptr)
{
	...
	int i=0;
  	for (i=0; i < 25; i++)
		...
		next_arrival_time[4].tv_nsec= next_arrival_nanoseconds%1000000000;
    }
}
...
void *polling_server( void *ptr)
{
	...
	int i=0;
  	for (i=0; i < 200; i++)
		...
		// we check if flag 6 has been set to true
		// if yes, we properly add the request to the queue and set
		// flag6 to false
      		pthread_mutex_lock(&mutex_task_6);
		if (flag6) 
		...
		// we check if flag 7 has been set to true
		// if yes, we properly add the request to the queue and set
		// flag7 to false
      		pthread_mutex_lock(&mutex_task_7);
		if (flag7) 
